...
def compute_targets(priors, gt_boxes, gt_labels, iou_threshold=0.35):
    """
    """
    ...
    valid_gt_idx = best_prior_iou >= 0.2
    best_prior_idx_filter = best_prior_idx[valid_gt_idx]
    if tf.shape(best_prior_idx_filter)[0] <= 0:
        gt_confs = tf.zeros(tf.shape(priors)[0], dtype=tf.int64)
        gt_locs = tf.zeros((tf.shape(priors)[0], 4), dtype=tf.float32)
        return gt_confs, gt_locs

    # Ensure each gt has at least one prior
    best_gt_iou = tf.tensor_scatter_nd_update(
        best_gt_iou,
        tf.expand_dims(best_prior_idx_filter, 1),
        tf.ones_like(best_prior_idx_filter, dtype=tf.float32))
